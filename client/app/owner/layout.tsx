...
export default function OwnerLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { name: "Menu Planner", href: "/owner/menu", icon: ChefHat },
    { name: "Pending Requests", href: "/owner/requests", icon: Inbox },
    { name: "Subscribers", href: "/owner/users", icon: Users },
    { name: "Billing Portal", href: "/owner/billing", icon: Wallet },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col md:flex-row font-sans selection:bg-indigo-500 selection:text-white">
      ...
      <header className="md:hidden sticky top-0 z-40 w-full border-b border-slate-800 bg-slate-950/80 backdrop-blur-md flex items-center justify-between px-4 h-16">
        <div className="flex items-center gap-2">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-indigo-600 shadow-[0_0_15px_rgba(99,102,241,0.3)] text-lg">
            🍱
          </div>
          <span className="text-base font-bold tracking-wider text-white">TFNS Service</span>
        </div>
        ...
        <div className="md:hidden fixed inset-x-0 top-16 z-30 border-b border-slate-800 bg-slate-950 px-4 py-3 space-y-1 shadow-2xl">
          ...
          <div className="pt-4 border-t border-slate-800 flex items-center justify-between px-4">
            <div className="flex flex-col">
              ...
              <span className="text-xs text-slate-500 uppercase">{user?.prefix} Kitchen</span>
            </div>
            <button
      ...
      <aside className="hidden md:flex flex-col w-64 border-r border-slate-900 bg-slate-950/50 backdrop-blur-md p-6 shrink-0 relative z-20">
        ...
        <nav className="flex-1 space-y-1.5">
          {navItems.map((item) => {
            // Check if page path matches or sub-paths match (e.g. /owner/users/[id] will still highlight 'Subscribers')
            const isActive = pathname.startsWith(item.href);
            const Icon = item.icon;
            return (
              <Link
        ...
        <div className="border-t border-slate-900 pt-6 flex flex-col gap-4">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-sm font-black text-indigo-400">
              {user?.prefix || "KP"}
            </div>
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[9px] text-slate-500 font-mono tracking-wider mt-0.5 uppercase">
                {user?.prefix ? `${user.prefix} Kitchen` : "Owner Portal"}
              </span>
            </div>
          </div>
          <button
